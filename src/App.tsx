...
export default function App() {
  ...
  const filteredTutorials = useMemo(() => {
    ...
    return tutorials.filter((tutorial) => {
      ...
      const matchesTuning = tuning === "All" || tutorial.tuning === tuning;
  ...
  const activeTutorial =
    tutorials.find((item) => item.id === activeTutorialId) ?? null;
          ...
          {filteredTutorials.length === 0 ? (
            <div className="mt-8 rounded-3xl border border-dashed border-slate-700 bg-slate-900/60 p-10 text-center">
              ...
              <p className="mt-2 text-sm text-slate-400">
                Try clearing the search or adjusting the difficulty, instrument,
                tuning, or category.
              </p>
            </div>
          ) : (
